...
function shiftLetter(letter, shift) {
    ...
    const shiftedCode = ((asciiCode - 65 + shift) % 26) + 65;
...
function caesarCipher(message, shift) {
    ...
    for (let char of message) {
        if (char === ' ') {
            ...
            const shiftedCode = ((asciiCode - 65 + shift) % 26) + 65;
            shiftedMessage += String.fromCharCode(shiftedCode);
        }
    }
...
function shiftByLetter(letter, letterShift) {
    if (letter === ' ') return ' ';
    const shiftValue = letterShift.charCodeAt(0) - 65;
    const letterCode = letter.charCodeAt(0);
    const shiftedCode = ((letterCode - 65 + shiftValue) % 26) + 65;
    return String.fromCharCode(shiftedCode);
}
...
function vigenereCipher(message, key) {
    ...
    for (let i = 0; i < message.length; i++) {
        ...
        if (char === ' ') {
            ...
            let shiftChar = key[keyIndex % key.length];
            let shiftValue = shiftChar.charCodeAt(0) - 65;
            let letterCode = char.charCodeAt(0);
            let shiftedCode = ((letterCode - 65 + shiftValue) % 26) + 65;
...
function scytaleCipher(message, shift) {
    ...
    const numColumns = message.length / shift;

    for (let i = 0; i < message.length; i++) {
        const index = Math.floor(i / shift) + numColumns * (i % shift);
...
function scytaleDecipher(message, shift) {
    const numColumns = message.length / shift; 
    const decodedMessage = new Array(message.length); 
  
    for (let i = 0; i < message.length; i++) {
        const index = Math.floor(i / numColumns) + shift * (i % numColumns);
        decodedMessage[i] = message[index];
    }
